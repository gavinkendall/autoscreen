...
namespace AutoScreenCapture
{
    ...
    public class Settings
    {
        ...
        public void Initialize(FileSystem fileSystem)
        {
            ...
            _versionCollection.Add(new Version(CODENAME_BLADE, "2.4.2.2")); // Application settings AllowUserToConfigureEmailSettings and AllowUserToConfigureFileTransferSettings disable respective menu items instead of hiding menu items. Default triggers added (and disabled) "Email Screenshots" and "Upload With SFTP". "New Region" added to Region Select system tray icon menu so you can perform a region selection and have the Add Region form open based on the provided region selection. Fixed the -show command line option so that any triggers using the ShowSystemTrayIcon action are enabled. Fixed the -hide command line option so that any triggers using the ShowSystemTrayIcon action are disabled. Screenshots uploaded to an SFTP server now retain folder structure based on Macro. File Transfer Settings includes new option "Connecting to a Linux server" to determine how the filepath should be handled on the SFTP server when splitting the path for sub-folder creation.
